document.addEventListener('DOMContentLoaded', function() {
    ...
    if(deleteBtns && confirmModal) {
        deleteBtns.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                document.getElementById('lockForm').action = `/admin/admin-user/${id}/lock`;
    ...
    statusToggles.forEach(toggle => {
        toggle.addEventListener('click', function(e) {
            ...
            if (action === 'lock') {
                ...
                document.getElementById('unlockForm').action = `/admin/admin-user/${id}/unlock`;
    ...
    if(removeBtns && removeModal) {
        removeBtns.forEach(btn => {
            btn.addEventListener('click', function() {
                const id = this.dataset.id;
                document.getElementById('removeForm').action = `/admin/admin-user/${id}`;
    ...
    if(promoteBtns && promoteModal) {
        promoteBtns.forEach(btn => {
            btn.addEventListener('click', function() {
                const id = this.dataset.id;
                document.getElementById('promoteForm').action = `/admin/admin-user/${id}/promote`;
    ...
    let rows = Array.from(document.querySelectorAll('.admin-row'));
    ...
    function applyFilters() {
        ...
        if (sortBy === 'balance') {
            visibleRows.sort((a, b) => {
                const balA = parseInt(a.dataset.balance) || 0;
                const balB = parseInt(b.dataset.balance) || 0;
                return balB - balA;
            });
        } else if (sortBy === 'new') {
            visibleRows.sort((a, b) => {
                const dateA = new Date(a.dataset.date).getTime() || 0;
                const dateB = new Date(b.dataset.date).getTime() || 0;
                return dateB - dateA;
            });
        }
    ...
    if (editId && pageTitleAdd && breadcrumbCurrentAdd && submitBtnTextAdd) {
        pageTitleAdd.textContent = 'Chỉnh sửa Admin';
        breadcrumbCurrentAdd.textContent = 'Chỉnh sửa';
        submitBtnTextAdd.textContent = 'Cập nhật Admin';
