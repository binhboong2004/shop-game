...
@section('title', $wheel->name . ' - ShopNickVN')
...
<div class="mb-12 mt-6">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ url('vongquay') }}" class="ml-1 text-sm font-medium text-slate-700 hover:text-[#E70814] md:ml-2 dark:text-slate-400 dark:hover:text-white transition-colors">Vòng quay may mắn</a>
                </div>
            </li>
            <li>
                <div class="flex items-center">
                    <span class="material-symbols-outlined text-slate-400">chevron_right</span>
                    <span class="ml-1 text-sm font-medium text-slate-500 md:ml-2 dark:text-slate-400">{{ $wheel->name }}</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="mb-8 text-center">
        <h1 class="text-3xl font-bold tracking-tight text-slate-900 dark:text-white sm:text-4xl mb-4 uppercase">
            {{ $wheel->name }}
        </h1>
        <p class="text-lg text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">
            {{ $wheel->description ?? 'Thử vận may ngay hôm nay với hàng ngàn phần quà cực chất đang chờ đón bạn. Lượt quay không giới hạn!' }}
        </p>
    </div>
        ...
        <div class="lg:col-span-7 xl:col-span-8 flex flex-col items-center">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-4 bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 w-full max-w-md justify-between">
                <div class="text-center sm:text-left">
                    ...
                    <p class="text-xl font-bold text-[#E70814]">{{ Auth::check() ? number_format(Auth::user()->balance) : 0 }} đ</p>
                </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                ...
                <div class="p-5 text-sm text-slate-600 dark:text-slate-400 space-y-3">
                    <ul class="list-disc pl-5 space-y-2">
                        <li>Mỗi lượt quay trị giá <strong class="text-green-500">{{ number_format($wheel->price) }}đ</strong> và sẽ trừ trực tiếp vào số dư tài khoản.</li>
                        <li>Trúng <strong>Vật phẩm</strong> sẽ được cộng trực tiếp vào ví sau 1-3 phút.</li>
                        <li>Trúng <strong>Tài khoản</strong> sẽ nhận được thông tin đăng nhập trong phần "Lịch sử mua hàng".</li>
