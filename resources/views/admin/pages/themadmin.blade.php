...
@section('title', 'Thêm Admin Nới')

@section('content')
<!-- Page Header -->
<div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
    <div>
        <h2 class="text-2xl font-bold text-white">{{ isset($user) ? 'Chỉnh sửa Admin' : 'Thêm Admin Mới' }}</h2>
        <nav class="text-sm font-medium text-gray-400 mt-1">
            <ol class="list-reset flex">
                ...
                <li><a href="{{ route('admin.taikhoanadmin') }}" class="text-[#E70814] hover:underline">Tài khoản Admin</a></li>
                <li><span class="mx-2">/</span></li>
    ...
    <div class="flex items-center gap-3">
        <a href="{{ route('admin.taikhoanadmin') }}" class="bg-[#20222a] hover:bg-[#2a2d35] border border-[#2a2d35] text-white px-4 py-2 rounded-md transition-colors flex items-center gap-2 text-sm font-medium">
...
<div class="bg-[#20222a] rounded-lg border border-[#2a2d35] p-6 mb-6 transform transition-all hover:border-gray-500/50">
    ...
    @endif
    <form action="{{ isset($user) ? route('admin.admin.update', $user->id) : route('admin.themadmin.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @if(isset($user))
            @method('PUT')
        @endif
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
                <h3 class="text-lg font-bold text-white mb-5 border-b border-[#2a2d35] pb-2 flex items-center gap-2">
                    <span class="material-symbols-outlined text-[#E70814]">admin_panel_settings</span>
                    Thông tin Admin
                </h3>
                
                <div class="mb-5">
                    <label class="block text-sm font-medium text-gray-300 mb-2">Họ và Tên <span class="text-[#E70814]">*</span></label>
                    <input type="text" name="name" value="{{ old('name', $user->name ?? '') }}" class="w-full bg-[#13131A] border border-[#2a2d35] text-white text-sm rounded-md focus:ring-1 focus:ring-[#E70814] focus:border-[#E70814] hover:border-gray-500 transition-colors block px-3 py-2.5 outline-none" placeholder="Nhập họ và tên admin" required>
                </div>
        ...
        <div class="mt-8 pt-6 border-t border-[#2a2d35] flex justify-end gap-3">
            <a href="{{ route('admin.taikhoanadmin') }}" class="px-6 py-2.5 rounded-md border border-[#2a2d35] text-white hover:bg-[#2a2d35] transition-colors font-medium">Hủy bỏ</a>
            <button type="submit" id="submitBtn" class="px-6 py-2.5 rounded-md bg-[#E70814] hover:bg-[#ff0f1e] text-white transition-colors font-medium shadow-[0_2px_10px_rgba(231,8,20,0.3)] flex items-center gap-2">
                ...
                <span id="submitBtnText">{{ isset($user) ? 'Cập nhật Admin' : 'Lưu Admin' }}</span>
            </button>
        </div>
    </form>
</div>
